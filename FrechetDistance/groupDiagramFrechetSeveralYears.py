...
# Initalize Data
migration_months = ["02", "03", "04", "05", "07"]
years = ["2014", "2015", "2016"]
distance_between_points = 100000  #meters
index_of_lat = 4
index_of_long = 3
index_of_date = 2
    ...
    if True: # cluster_open[2] == cluster_closed[2]:
        ...
        if cluster_extended[2] == cluster_closed[2] and monitoring:
            ...
            if G_last_segment != G_last_segment_extended or not monitoring_2:
                # print "Reson for being relavant: some event in 2d distane", a, b
                return False
            else:
                return True
        else:
...
def read_data(data_set, individual, distance_between_points, index_of_lat, index_of_long,
                 index_of_date, index_of_year_start, index_of_year_end, index_of_month_start,
                 index_of_month_end, migration_months):
    trajectory = []
    trajectory_with_date = []
    with open(data_set) as csvFile:
        data = csv.reader(csvFile)
        next(data)
        first_row = next(data)
        for row in data:
            if row[32] == individual: # and float(row[14]) < 15:
                trajectory.append([float(first_row[index_of_lat]), float(first_row[index_of_long])])
                trajectory_with_date.append([float(first_row[index_of_lat]), float(first_row[index_of_long]),
                                             first_row[index_of_date][index_of_year_start:index_of_year_end]])
                count = 0
                break
        for row in data:
            try:
                if row[32] == individual and float(row[index_of_long]) < 100:  # and float(row[14]) < 15:
                    ...
                    if dist_travelled > distance_between_points \
                            ...
                            and new_point[0] > 51 and new_point[1] > 5:
                        trajectory.append([float(row[index_of_lat]), float(row[index_of_long])])
                        trajectory_with_date.append([float(row[index_of_lat]), float(row[index_of_long]),
                                                     row[index_of_date][index_of_year_start:index_of_year_end]])
                        count = count + 1
            except ValueError:
                "Incorrect Data Format"
    return trajectory, trajectory_with_date



#tracks = ["GWFG_2015_409"]
# tracks = ["711"]

for individual in tracks[1:]:
    global_time_start = time.time()
    time_read_start = time.time()
    data = read_data(data_set, individual, distance_between_points, index_of_lat, index_of_long,
                     index_of_date, index_of_year_start, index_of_year_end, index_of_month_start,
                     index_of_month_end, migration_months)
    animal = data[0]
    ...
    for year in years:
        all_data.append([animal_sample_additional[date] for date in range(0, len(animal_sample_additional))
                          if animal_sample_additional[date][2] == year])

    year_with_almost_no_data = [year for year in all_data if len(year) < 10]
    if len(year_with_almost_no_data) == 0:
        print "Name of individual", individual
        lines = []
        ...
        solution_animal = setcover.SetCover(covering_matrix, costs, maxiters=100)
        ...
        if segmentation_1 and segmentation_2:
            html = individual + "_migration_" + "segmentation_first_and_second_type_" + str(dist_tol) + "_" + str(resolution)

        elif segmentation_1 and not segmentation_2:
            html = individual + "_migration_" + "segmentation_first_type_" + str(dist_tol) + "_" + str(resolution)

        elif not segmentation_1 and segmentation_2:
            html = individual + "_migration_" + "segmentation_second_type_" + str(dist_tol) + "_" + str(resolution)

        else:
            html = individual + "_migration_" + "no_segmentation" + str(dist_tol) + "_" + str(resolution)

        kml.save(html+"_time_evaluation.kml")
